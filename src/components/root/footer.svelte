...
<footer>
	<Container
		className={variant == 'home'
			? 'bg-primary-500/80 transition-colors dark:bg-slate-800'
			: 'mt-16 lg:mt-32'}
	>
		<div class="{variant === 'home' && 'py-16'} py-10">
			<div class="flex flex-col items-center justify-between gap-6 md:flex-row md:items-start">
				<div
					class="flex flex-col justify-center self-stretch text-sm text-zinc-800 transition-colors dark:text-zinc-200"
				>
					<div class="flex w-full justify-center flex">
						<img src="{base}/images/name.svg" alt="name logo" class="h-16" />
					</div>
				</div>
				...
				>
					<p
						class="flex flex-row text-center items-center justify-center text-zinc-700 transition-colors md:text-left dark:text-zinc-300"
					>
						Made with <a
							class="ps-1 relative flex items-center text-sm font-medium text-sky-500 transition-colors dark:text-sky-400"
							target="_blank"
							href="https://skeleton.dev">Skeleton</a
						><svg
							class="fill-token h-[24px] md:h-[32px] ps-2"
					...
					<p class="flex justify-center">The UI toolkit for Svelte and Tailwind.</p>
				</div>

				<div
					class="flex w-full justify-center flex-shrink-0 gap-14 md:w-auto md:justify-start lg:gap-20"
				>
					<div class="flex justify-center">
						<Socials orientation="vertical"></Socials>
					</div>
				</div>
			</div>
		</div>
